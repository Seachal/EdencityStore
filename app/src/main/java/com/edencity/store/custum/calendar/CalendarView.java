...
public class CalendarView extends LinearLayout {
    private MyMediumTextView title;
    ...
    private int fourthMonth = -1;//卡片第四个月（中间）
    private int fifithMonth = -1;//卡片第五个月（中间）
    private int sixthMonth = -1;//卡片第六个月（中间）
    private int seventhMonth = -1;//卡片第七个月（中间）
    private int eighthMonth = -1;//卡片第八个月（中间）
    private int ninethMonth = -1;//卡片第九个月（中间）
    private int tenthMonth = -1;//卡片第十个月（中间）
    private int eleventhMonth = -1;//卡片第十一个月（中间）
    private int twelvethMonth = -1;//卡片第十二个月（中间）
    ...
    public CalendarView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        titleColor = ta.getColor(R.styleable.MyCalendar_titleColor, Color.parseColor("#333333"));
        titleSize = (int) ta.getDimension(R.styleable.MyCalendar_titleSize, 18);
        enableSelectColor = ta.getColor(R.styleable.MyCalendar_dayInMonthColor,
                Color.parseColor("#333333"));
        disableSeletColor = ta.getColor(R.styleable.MyCalendar_dayOutMonthcolor,
                context.getResources().getColor(R.color.transparent40_white));
    ...
    private void bindView(Context context) {
        ...
        title = (MyMediumTextView) view.findViewById(R.id.calendar_title);
        left = (ImageView) view.findViewById(R.id.left);
        right = (ImageView) view.findViewById(R.id.right);
        left.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mainAdapter.getPosition()>0){
                    recyclerView.scrollToPosition(mainAdapter.getPosition()-1);
                    mainAdapter.notifyDataSetChanged();
                    title.setText(titles.get(mainAdapter.getPosition()-1));
                }else if (mainAdapter.getPosition()==0){
        ...
        right.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mainAdapter.getPosition()<titles.size()-1){
                    recyclerView.scrollToPosition(mainAdapter.getPosition()+1);
                    mainAdapter.notifyDataSetChanged();
                    title.setText(titles.get(mainAdapter.getPosition()+1));
                }else if(mainAdapter.getPosition()== titles.size()-1){
                    recyclerView.scrollToPosition(titles.size()-1);
                    mainAdapter.notifyDataSetChanged();
                    title.setText(titles.get(titles.size()-1));
                }
            }
        });
    ...
    public void renderCalendar(String time) {
        ...
        fourthMonth = months.get(3).getCells().get(20).getMonth();
        fifithMonth = months.get(4).getCells().get(20).getMonth();
        sixthMonth = months.get(5).getCells().get(20).getMonth();
        seventhMonth = months.get(6).getCells().get(20).getMonth();
        eighthMonth = months.get(7).getCells().get(20).getMonth();
        ninethMonth = months.get(8).getCells().get(20).getMonth();
        tenthMonth = months.get(9).getCells().get(20).getMonth();
        eleventhMonth = months.get(10).getCells().get(20).getMonth();
        twelvethMonth = months.get(11).getCells().get(20).getMonth();
            ...
            subRvAdapter.setOnItemClickListener(new OnItemClickListener() {
                ...
                public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                    ...
                    if (date.getMonth() == secondMonth
                            ...
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == fourthMonth)
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == fifithMonth)
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == sixthMonth)
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == seventhMonth)
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == eighthMonth)
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == ninethMonth)
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == tenthMonth)
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == eleventhMonth)
                            || (date.getDate() <= dateFromServer.getDate() && date.getMonth() == twelvethMonth)
                    ) {
                        //可点击数据
                        if (isSelectingSTime) {
                            //正在选择开始时间
    ...
    private class SubRvAdapter extends BaseQuickAdapter<Date, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder helper, Date date) {
            ...
            if (sDateTime != null && eDateTime != null && date.getTime() > sDateTime.getTime() && date.getTime() < eDateTime.getTime()) {
                if (date.getDay() == 6) {
                    ((CalendarDayTextView) helper.getView(R.id.calendar_day_tv)).setColor();
